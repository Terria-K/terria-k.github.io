...
export const GET: APIRoute = async (ctx) => {
  ...
  let findIfExists = await db
    .select()
    .from(User)
    .where(or(eq(User.email, name), eq(User.username, name)));

  let user = findIfExists.at(0);

  if (!user) {
    return new Response(
      "Account does not existed with this email or username.",
      { status: 401 },
    );
  }
  ...
  return new Response(
    "The verification token has been resend to your email. Please check your inbox or spam.",
  );
};
